...
/*
  if (0) printf("f:theFilter        = %p \n", theFilter);
  if(theFilter!=NULL){
    if (0) printf("f:theFilter->index = %u \n",theFilter->index);
  }
*/
  ...
  if(theFilter==0){
    if (0) printf("No filter present. EXIT.\n");
    return(1);
  }
...
//    if (0) printf("Filter present, all frames match.EXIT\n");
    return(1);
  }
...
//    if (0) printf("pkt.ip.proto = %d <> %d theFilter.ip_proto \n", ip_hdr->ip_p,theFilter->ip_proto);
